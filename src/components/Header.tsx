...
const Header = () => {
  ...
  return (
    <header className={`fixed w-full z-[10000] transition-all duration-500 ease-out ${isScrolled
      ? 'bg-black/30 backdrop-blur-3xl border-b border-white/10 py-4 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)]'
      : 'bg-transparent py-6'
      }`}>
      {/* Dynamic gradient overlay */}
      <div
        className={`absolute inset-0 transition-opacity duration-1000 pointer-events-none ${isScrolled ? 'opacity-0' : 'opacity-100'}`}
        style={{
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative">
        <div className="flex justify-between items-center">
          ...
          <nav className={`!hidden lg:!flex items-center space-x-1 relative z-50`}>
            {navItems.map((item, index) => (
              <div key={item.name} className="relative group">
                <Link
                  to={item.path}
                  className="relative px-5 py-2 text-sm font-medium text-gray-300 hover:text-white transition-all duration-300 rounded-lg group-hover:bg-white/5 hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] block"
                >
                  {item.name}
                  {/* Underline effect */}
                  <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-blue-400 to-purple-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left shadow-[0_0_10px_rgba(59,130,246,0.5)]"></span>
                </Link>
              </div>
            ))}
          </nav>
          ...
          <div className="hidden lg:block relative z-50 flex-shrink-0">
            <Link
              to="/#contact"
              className={`relative inline-flex items-center px-8 py-4 rounded-2xl font-bold text-white transition-all duration-700 transform hover:scale-110 hover:rotate-1 group overflow-hidden shadow-2xl ${isScrolled
            ...
            >
              ...
              <span className="relative z-10 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent font-extrabold tracking-wide drop-shadow-md">
                Get Started
              </span>
          ...
          <button
            ...
            className={`lg:hidden p-4 rounded-2xl transition-all duration-700 transform hover:scale-125 hover:rotate-180 group overflow-hidden relative z-50 ${isScrolled
        ...
        {isOpen && (
          <div className="lg:hidden mt-8 bg-white/95 backdrop-blur-3xl rounded-3xl shadow-2xl border border-gray-200/50 overflow-hidden animate-slide-in-down relative z-50"
            style={{
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-blue-500/50 via-purple-500/50 via-pink-500/50 to-transparent transition-opacity duration-1000 ${isScrolled ? 'opacity-100' : 'opacity-0'
        }`}></div>
    </header>
  );
};
